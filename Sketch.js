...
function setup() {
  ...
  for (let i = 0; i < numBlobs; i++) {
    ...
    while (isOverlapping) {
      ...
      x = random(width - blobSize, width - blobSize);
      y = random(height - blobSize, height - blobSize);
    ...
    blobs.push(
      new Jelly(x, y, blobSize, elasticity, noiseScale, noiseStrength)
    ); // Create a new Jelly object with random position and other parameters, and add it to the blobs array
  }
}
  ...
  display() {
    ...
    for (let i = 0; i < numPoints; i++) {
      ...
      let noiseValue = noise(
        (this.x + cos(angle) * this.size) * this.noiseScale,
        (this.y + sin(angle) * this.size) * this.noiseScale,
        noiseOffset * this.noiseScale
      );
      let offset = map(
        noiseValue,
        0,
      ...
      let prevAngle = map(
        (i - 1 + numPoints) % numPoints,
        0,
        numPoints,
        0,
        TWO_PI
      ); // Calculate angle of the previous point
